...
Base Case: Single vehicle type (Van), CVRP with DOT constraints
Heuristics:
...
# PARAMETERS
# ─────────────────────────────────────────────
VAN_CAPACITY   = 3200          # ft³
UNLOAD_RATE    = 0.03          # min/ft³
MIN_UNLOAD     = 30            # minutes
SPEED          = 40            # mph
SPEED_MPM      = 40 / 60       # miles per minute
...
WINDOW_OPEN    = 8  * 60       # 8:00 am
WINDOW_CLOSE   = 18 * 60       # 6:00 pm
...
DEPOT_ID = 20  # zipid for Wilmington 01887
...
def evaluate_route(order_ids_seq, day):
    """
    Evaluate a single-day route. Returns info dict or None if infeasible.
    """
    ...
    for i, loc in enumerate(loc_seq[1:], 1):
        ...
        if oid is not None:
            ...
            if depart_time > WINDOW_CLOSE:
                return None  # all orders must finish unloading by 6pm
    ...
    return {
        ...
        'drive_time': drive_accum,
        'duty_time': duty_accum,
...
def nearest_neighbor_routes(day):
    """Build routes using nearest-neighbor greedy insertion."""
        ...
        while unassigned:
            ...
            for oid in unassigned:
                ...
                if depart > WINDOW_CLOSE:
                    continue  # unloading would finish after 6pm
                ...
                if d < best_dist:
                    best_dist = d
                    best_oid  = oid
                    best_info = (loc, arrive, arrive + ul, _drive, _duty_after, ul)
...
def clark_wright_routes(day):
    """
    Build routes using the Clark-Wright parallel savings algorithm.

    1.  Start with each order on its own route:  depot → order → depot
    2.  Compute savings  s(i,j) = d(depot,i) + d(depot,j) − d(i,j)  for all pairs
    3.  Sort savings descending
    4.  For each pair, merge if:
        a) i and j are on different routes
        b) i is the last stop of its route  OR  j is the first stop of its route
           (so we can concatenate the two routes)
        c) The merged route is feasible (capacity + full time simulation)
    """
...
# ═════════════════════════════════════════════════
# HELPER: build & improve routes from a construction
# ═════════════════════════════════════════════════

def build_and_improve(raw_routes, day):
    """Run 2-opt on each route and return list of evaluated route dicts."""
    results = []
...
print("\n" + "=" * 60)
print("  Construct & improve single-day routes")
print("=" * 60)
...
for day in DAY_ORDER:
    ...
    # ── Construction A: Nearest Neighbor ──
    ...
    # ── Construction B: Clark-Wright Savings ──
...
all_routes = []
for day in DAY_ORDER:
    all_routes.extend(day_routes_dict[day])
...
# Min vehicles (max concurrent)
events = []
for rw in route_windows:
    events.append((rw['dispatch_abs'], +1))
    events.append((rw['finish_abs'],   -1))
events.sort()
...
# Min drivers (greedy assignment with DOT break)
...
print(f"{'Day':<6} {'Routes':>7} {'Orders':>7} {'Miles':>10} {'Avg Miles':>10}")
print("-" * 45)
...
for day in DAY_ORDER:
    ...
    print(f"{day:<6} {n_routes:>7} {n_orders:>7} {miles:>10,.1f} {avg:>10,.1f}")

print("-" * 45)
print(f"{'TOTAL':<6} {len(all_routes):>7} "
